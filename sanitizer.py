    ...
    if add_noise:
      ...
      with tf.control_dependencies([privacy_accum_op]):
        saned_x = utils.AddGaussianNoise(tf.reduce_sum(x, 0),
                                         sigma * l2norm_bound*0.9)
    else:
      saned_x = tf.reduce_sum(x, 0)
    return saned_x

class MyAmortizedGaussianSanitizer(object):
  """Sanitizer with Gaussian noise and amoritzed privacy spending accounting.
  ...
  def sanitize(self, x, eps_delta, epoch=0, min_clipping=0, clipping_decay=False, dd=None, sigma=None, 
               option=ClipOption(None, None), tensor_name=None,
               num_examples=None, add_noise=True):
    """Sanitize the given tensor.
    ...
    if sigma is None:
        # pylint: disable=unpacking-non-sequence
        eps, delta = eps_delta
        with tf.control_dependencies([tf.Assert(tf.greater(eps, 0),
                                    ["eps needs to be greater than 0"]),
                                    tf.Assert(tf.greater(delta, 0),
                                    ["delta needs to be greater than 0"])]):
            # The following formula is taken from
    ...
    if l2norm_bound is None:
        ...
        new_l2norm_bound = l2norm_bound
    if clip:
        lower_bound = min_clipping
        upper_bound=l2norm_bound
        if clipping_decay:
            upper_bound = tf.divide(l2norm_bound, tf.sqrt(epoch+1.0))
        
        # x, new_l2norm_bound = utils.MyBatchClipByL2norm(x, upper_bound, lower_bound)
        x = utils.BatchClipByL2norm(x, l2norm_bound)
    ...
    if add_noise:
        if num_examples is None:
            num_examples = tf.slice(tf.shape(x), [0], [1])
        if dd is None:
            saned_x, diff = utils.AddMyGaussianNoise(tf.reduce_sum(x, 0),
                                        sigma * new_l2norm_bound)
        else:
            saned_x, diff = utils.AddMyGaussianNoise( tf.reduce_sum(x, 0),
                                        tf.multiply(tf.multiply(sigma, new_l2norm_bound), dd*0.9) )
    else:
        saned_x = tf.reduce_sum(x, 0)
        diff = tf.zeros(tf.shape(saned_x))
    return saned_x, diff