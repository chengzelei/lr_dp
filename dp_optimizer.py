...
class DPGradientDescentOptimizer(tf.train.GradientDescentOptimizer):
  ...
  def compute_sanitized_gradients(self, loss, var_list=None,
    ...
    for px_grad, v in zip(px_grads, var_list):
      ...
      grads_noises.append(tf.subtract(sanitized_grad, tf.reduce_sum(px_grad, 0)))
...
class MyDPGradientDescentOptimizer(tf.train.GradientDescentOptimizer):
  """Differentially private gradient descent optimizer.
  """

  def __init__(self, learning_rate, eps_delta, sanitizer,
               sigma=None, epoch=0, min_clipping=0, clipping_decay=False, use_locking=False, name="MyDPGradientDescent"):
    """Construct a differentially private gradient descent optimizer.
    ...
    super(MyDPGradientDescentOptimizer, self).__init__(learning_rate,
                                                     use_locking, name)
    ...
    self._clipping_decay = clipping_decay
  ...
  def compute_sanitized_gradients(self, loss, var_list=None,
    ...
    for px_grad, v in zip(px_grads, var_list):
        ...
        new_noise_coeff=1.0
        if add_noise:
            new_noise_coeff = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tensor_name+utils.NEW_NOISE_COEFF)[0]

        sanitized_grad_init, grad_noise = self._sanitizer.sanitize(
            px_grad, self._eps_delta, epoch=self._epoch, min_clipping=self._min_clipping, clipping_decay=self._clipping_decay,
            dd=new_noise_coeff , sigma=self._sigma,
            tensor_name=tensor_name, add_noise=add_noise,
            num_examples=tf.slice(tf.shape(px_grad), [0], [1]))

        if add_noise:
            pre_noise_name = tensor_name + utils.NOISE_PREV_SUFFIX
            pre_noise = tf.get_collection(utils.PREV_EPOCH_DATA_COLLECTION_KEY, scope=pre_noise_name)[0]
            reuse_noise_coeff = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tensor_name+utils.REUSE_NOISE_COEFF)[0]
            pre_noise_reuse = tf.multiply(pre_noise, reuse_noise_coeff)

            sanitized_grad = tf.add(sanitized_grad_init, pre_noise_reuse)

            cur_noise_name = tensor_name + utils.NOISE_CUR_SUFFIX
            cur_noise = tf.get_collection(utils.PREV_EPOCH_DATA_COLLECTION_KEY, scope=cur_noise_name)[0]
            grads_noises.append(tf.assign(cur_noise, tf.add(grad_noise, pre_noise_reuse)))
        else:
            sanitized_grad = sanitized_grad_init

        sanitized_grads.append(sanitized_grad)

    return sanitized_grads, grads_noises

  def minimize(self, loss, global_step=None, var_list=None, name=None, add_noise=True):
    """Minimize using sanitized gradients.
    ...
    sanitized_grads, grads_noises = self.compute_sanitized_gradients(
          loss, var_list=var_list, add_noise=add_noise)
    ...
    return apply_grads, grads_noises